...
print_modname() {
  ...
  ui_print "* by @shadowstep and @My Name is ShaoXIa *"
  ui_print "*            at xda-developers           *"
  ui_print "******************************************"
}
...
on_install() {
  ...
  cp -r /system/priv-app/TeleService/* $MODPATH/system/priv-app/TeleService
  sleep 5
  if [ -f "$MODPATH/system/priv-app/TeleService/TeleService.apk" ];then
	  ui_print "Files copied successfully!"
  else
	  abort "Files not copied, error!"
  fi
